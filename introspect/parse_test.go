...
<node>
  <interface name="org.bluez.Device1">
    ...
    <annotation name="org.deepin.DBus.configuration.DefaultServiceName" value="org.deepin.abc"/>
    <annotation name="org.deepin.DBus.configuration.DefaultObjectPath" value="/org/deepin/dabc"/>
  </interface>
</node>
`

func TestAnnotationDeepin(t *testing.T) {
	C.Convey("Create a reader ", t, func() {
		reader := bytes.NewBufferString(testXML)
		ninfo, err := Parse(reader)
		C.So(err, C.ShouldBeNil)
		C.Convey("Check org.deepin.DBus.Configuration", func() {
			C.So(len(ninfo.Interfaces), C.ShouldEqual, 1)
			ifc := ninfo.Interfaces[0]
			C.So(ifc.Name, C.ShouldEqual, "org.bluez.Device1")

			C.So(ifc.GetAnnotation(AnnotationConfigurationDefaultServiceName), C.ShouldEqual, "org.deepin.abc")
			C.So(ifc.GetAnnotation(AnnotationConfigurationDefaultObjectPath), C.ShouldEqual, "/org/deepin/dabc")
		})
	})
}
